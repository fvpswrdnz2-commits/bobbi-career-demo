...
- 一句话搜索由后端依据词典确定性提取标签、时间和时长。
- 非编号查询按照教育背景 20%、求职经历 50%、档期可约度 30%计算分数。
- 匹配理由只从公开标签和有效档期生成，不读取证明材料或其他私密字段。
